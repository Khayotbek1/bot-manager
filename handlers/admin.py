...
async def export_menu(message: Message, state: FSMContext):
    ...
    await message.answer("📤 <b>Export bo‘limi</b>", reply_markup=export_kb())
...
@router.message(
    AdminSection.export,
    F.text.regexp(r"^\d{2}\.\d{2}\.\d{4}\s\d{2}\.\d{2}\.\d{4}$")
)
...
@router.message(
    AdminSection.statistics_menu,
    F.text.in_(["👥 Jami", "📅 Bugun", "🗓 Oxirgi 7 kun", "📉 Joriy oy"])
)
async def global_stats(message: Message):
    stats = await get_full_statistics()

    mapping = {
        "👥 Jami": ("👥 Jami", stats["total_registered"], stats["total_left"]),
        "📅 Bugun": ("📅 Bugun", stats["today_joined"], stats["today_left"]),
        "🗓 Oxirgi 7 kun": ("🗓 Oxirgi 7 kun", stats["week_joined"], stats["week_left"]),
        "📉 Joriy oy": ("📉 Joriy oy", stats["month_joined"], stats["month_left"]),
    }
...
@router.message(AdminSection.statistics_menu, F.text == "📢 Kanallar bo‘yicha")
...
@router.message(AdminSection.channel_list)
async def choose_channel(message: Message, state: FSMContext):
    ...
    await state.set_state(AdminSection.channel_detail)
...
async def post_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "📢 Xabarni yuboring (rasm,video,file,text):",
        reply_markup=cancel_post_kb()
    )
...
async def cancel_post(callback: CallbackQuery, state: FSMContext):
    ...
    # faqat post paytida ishlasin
    if current != AdminPostState.waiting_content:
...
async def post_send(message: Message, state: FSMContext):
    async with SessionLocal() as session:
        users = (await session.execute(
            select(User.telegram_id).where(User.is_registered.is_(True))
        )).scalars().all()
